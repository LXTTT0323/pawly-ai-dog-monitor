  ...
  return (
    <main className="app-shell">
      ...
      <section className="setup-wrap shell">
        <div className="setup-intro"><span className="eyebrow">Local beta setup</span><h1>One room.<br />Two screens.</h1><p>Any spare phone, tablet, or computer becomes the dog-aware camera. Your other device becomes the quiet dashboard. The 12-character room key is the only way into this beta room—treat it like a password.</p></div>
        <div className="setup-panel">
          <div className="setup-step"><span>1</span><div><h2>Open camera mode</h2><p>Use a modern browser and allow both camera and microphone. Sound can be turned off anytime without stopping video.</p><div className="room-code">{roomCode || "••••••••••••"}</div><div className="button-row"><a className="button button-primary" href={cameraUrl}>Open camera on this device</a><button className="button button-ghost" onClick={() => navigator.clipboard.writeText(cameraUrl)}>Copy camera link</button></div></div></div>
        ...
        <p className="beta-warning"><strong>Camera-device note:</strong> keep it plugged in and do not lock the screen. Pawly automatically switches to a nearly black standby screen while the camera stays active; tap it—or use “Wake camera display” from the dashboard—to show the preview again.</p>
      </section>
    </main>
  );
}
